...
export const CopyMeetingInfo = (props?: CopyMeetingInfoProps) => {
  ...
  return (
    <>
      {isSeparateHostLink ? (
        <>
          <Text style={style.urlTitle}>Room Id for Attendees</Text>
          <Spacer size={11} />
      ...
      {isHost ? (
        <>
          <Text style={style.urlTitle}>Room Id for Host</Text>
          <Spacer size={11} />
          <View style={style.container}>
            <View style={style.urlContainer}>
              <Text
...
export const ShareWithRoomAvailable = ({roomId, meetingTitle, attendeeId}) => {
  const {FpeShareComponent} = useCustomization(data => {
    let components: {
      FpeShareComponent?: React.ElementType;
    } = {};
