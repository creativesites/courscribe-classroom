...
import {ShareWithRoomAvailable} from '../components/Share';
...
const Create = () => {
  ...
  const [hasRoomAlready, setHasRoomAlready] = useState(false)
  const [attendeesRoomId, setAttendeesRoomId] = useState(null)
  ...
  useEffect(() => {
    ...
    const curriculumName1 = urlParams.get('curriculumName');
    const userId1 = urlParams.get('userId');
    const curriculumId1 = urlParams.get('curriculumId');
    const attendeesRoomId1 = urlParams.get('attendeesRoomId');
    const hostRoomId1 = urlParams.get('hostRoomId');
    if (!userId1) {
      // Close the window if userId is not present
      window.close();
    }
    if (curriculumName1 && userId1 && curriculumId1) {
      setCurriculumName(curriculumName1)
      onChangeRoomTitle(curriculumName1)
      setCurriculumId(curriculumId1)
      setUserId(userId1)
      if(attendeesRoomId1 && hostRoomId1){
          setAttendeesRoomId(attendeesRoomId1)
          setHostRoomId(hostRoomId1)
          setHasRoomAlready(true)
  ...
  return (
    <CreateProvider
      ...
      {hasRoomAlready? <ShareWithRoomAvailable meetingTitle={curriculumName} roomId={hostRoomId} attendeeId={attendeesRoomId}/>:
      (
        <>
        {!roomCreated ? (
        CreateComponent ? (
            ...
            <ScrollView contentContainerStyle={style.main}>
              <Card>
                <View>
                  ...
                  <Text style={style.heading}>Create A CourScribe Classroom</Text>
                  <Spacer size={40} />
                  <Input
                ...
                <View style={[style.btnContainer]}>
                  <PrimaryButton
                    ...
                    text={loading ? loadingText : "Create A Classroom Stream"}
                  />
                  <Spacer size={16} />
                  <LinkButton
                    text={joinWithRoomID}
